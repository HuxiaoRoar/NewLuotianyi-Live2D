...
function showMessage(text, timeout) {
    ...
    var wasHidden = !$msg.is(':visible') || parseFloat($msg.css('opacity')) <= 0.01;

    $msg.stop();

    // 只有气泡原本已经隐藏时，才归零。
    // 如果只是连续点击换文本，不归零，避免瞬移。
    if (wasHidden) {
        stopBubbleFloat(true);
    }
...
function hideMessage(timeout) {
    ...
    if (timeout === null) timeout = 5000;
    $('.message').delay(timeout).fadeOut(200, function () {
        stopBubbleFloat(true);
    });
}

function resetBubbleFloat(el) {
    if (!el) return;
...
function scheduleBubbleFloat() {
    var el = document.querySelector('.message');
    if (!el || !el.animate) return;

    // 已经在飘，或者已经安排了延迟启动，就不要重复安排
    if (el._poiBubbleFloatRunning || el._poiBubbleFloatTimer) return;

    el._poiBubbleFloatTimer = setTimeout(function () {
        el._poiBubbleFloatTimer = null;

        // 如果 6 秒后气泡已经隐藏了，就不启动
        var $msg = $('.message');
        if (!$msg.is(':visible') || parseFloat($msg.css('opacity')) <= 0.01) return;
...
function startBubbleFloat() {
    ...
    if (el._poiBubbleFloatRunning) return;

    el._poiBubbleFloatRunning = true;

    var state = el._poiBubbleFloatState || {
        x: 0,
        y: 0,
        r: 0
    };
    ...
    function makeTransform(s) {
        return 'translate(' + s.x.toFixed(2) + 'px, ' + s.y.toFixed(2) + 'px) rotate(' + s.r.toFixed(3) + 'deg)';
    }
    ...
    function nextFloat() {
        ...
        function randSigned(minAbs, maxAbs) {
            var v = rand(minAbs, maxAbs);
            return Math.random() < 0.5 ? -v : v;
        }

        var next = {
            x: clamp(state.x + randSigned(0.5, 1.2), -2, 2),
            y: clamp(state.y + randSigned(0.5, 1.2), -2, 2),
            r: clamp(state.r + randSigned(0.15, 0.5), -1, 1)
        };

        var duration = Math.round(rand(900, 1400));

        var anim = el.animate([
            { transform: makeTransform(state) },
            { transform: makeTransform(next) }
        ], {
            duration: duration,
            easing: 'ease-in-out',
            fill: 'forwards'
        ...
        el._poiBubbleFloatAnim = anim;

        anim.onfinish = function () {
            if (!el._poiBubbleFloatRunning) return;
            if (el._poiBubbleFloatAnim !== anim) return;

            state = next;
            el._poiBubbleFloatState = state;

            // 关键：把当前最终状态落到真实 style 上
            el.style.transform = makeTransform(state);

            // 关键：清掉这一段 fill: forwards 的动画残留
            anim.onfinish = null;
            anim.cancel();

            if (el._poiBubbleFloatAnim === anim) {
                el._poiBubbleFloatAnim = null;
            }
    ...
    // 确保开始时 style 和内部 state 一致
    el.style.transform = makeTransform(state);

    nextFloat();
}
function stopBubbleFloat(reset) {
    var el = document.querySelector('.message');
    if (!el) return;

    if (el._poiBubbleFloatTimer) {
        clearTimeout(el._poiBubbleFloatTimer);
        el._poiBubbleFloatTimer = null;
    }

    el._poiBubbleFloatRunning = false;

    if (el._poiBubbleFloatAnim) {
        el._poiBubbleFloatAnim.onfinish = null;
        el._poiBubbleFloatAnim.cancel();
        el._poiBubbleFloatAnim = null;
    }
...
(function () {
    ...
    var festivalContext = {
        ...
        // 中华人民共和国成立周年
        prc_years: currentYear - 1949,

        // 中国共产党成立周年
        cpc_years: currentYear - 1921,

        // 中国人民解放军建军周年
        pla_years: currentYear - 1927,

        //抗战胜利周年
        victory_years: currentYear - 1945,

        // 洛天依诞生周年
        luotianyi_years: currentYear - 2012
    };
    ...
    function collectWelcomeMessages(pool, value, context) {
        var list = [];
    ...
    for (var k = 0; k < festivals.length; k++) {
        var festivalDate = normalizeFestivalDate(festivals[k].selector);

        if (festivalDate === todayStr) {
    ...
    if (!text) {
        if (window.location.href == `${home_Path}`) {
            ...
            for (var m = 0; m < hourlies.length; m++) {
                var selector = hourlies[m].selector || '';
                var match = String(selector).trim().match(/^(\d{1,2})—(\d{1,2})$/);

                if (!match) continue;

                var startHour = parseInt(match[1], 10);
                var endHour = parseInt(match[2], 10);

                // 只接受标准时间段：0 <= start < end <= 24
                if (
                    isNaN(startHour) ||
                    isNaN(endHour) ||
                    startHour < 0 ||
                ...
                // 左闭右开：[startHour, endHour)
                // 例：t0-5 命中 0,1,2,3,4 点；t5-7 命中 5,6 点。
