...
function InitPoi() {
    ...
    if (window.innerWidth <= (parseInt(poilive2d_config.role_hide_width) || 0)) {
        return;
    }
    ...
    PIXI.live2d.Live2DModel.from(modelUrl).then(model => {
        ...
        // ==========================================
        // ★ POI 呼吸系统：Pixi 默认 / LPV ParamBreath Set
        // ==========================================

        // ==========================================
        // ★ POI 呼吸系统：Pixi 默认 / LPV ParamBreath Set
        // ==========================================
        const poiBreathControl = (function installPoiBreathControl(model, rawJson) {
            const im = model && model.internalModel;
            const core = im && im.coreModel;
            const modelName = typeof currentModel !== 'undefined' ? currentModel : '(unknown-model)';

            if (!im || !core) {
                //  console.warn('[Poi呼吸系统] 跳过：internalModel/coreModel 不存在');
                return null;
            }

            function readPoiFloatConfig(key, fallback, min, max) {
                const raw = typeof poilive2d_config !== 'undefined'
                    ? poilive2d_config[key]
                    : undefined;
            ...
            function getCoreParam(id, fallback = 0) {
                try {
                    const v = core.getParameterValueById(id);
            ...
            function getParamRange(id) {
                ...
                try {
                    if (typeof core.getParameterMinimumValueById === 'function') {
                        const v = core.getParameterMinimumValueById(id);
                        if (typeof v === 'number' && Number.isFinite(v)) min = v;
                    }
                } catch (e) { }

                try {
                    if (typeof core.getParameterMaximumValueById === 'function') {
                        const v = core.getParameterMaximumValueById(id);
                        if (typeof v === 'number' && Number.isFinite(v)) max = v;
                    }
                } catch (e) { }
            ...
            const autoBreathCtrl =
                controllers.AutoBreath ||
                controllers.autoBreath ||
                controllers.auto_breath ||
                null;

            const autoBreathExplicitDisabled = !!(
                autoBreathCtrl &&
                (
                    autoBreathCtrl.Enabled === false ||
                    autoBreathCtrl.enabled === false ||
                    autoBreathCtrl.Enabled === 0 ||
                    autoBreathCtrl.enabled === 0 ||
                    autoBreathCtrl.Enabled === 'false' ||
                    autoBreathCtrl.enabled === 'false'
                )
            ...
            const hasLPVSignal = !!(
                autoBreathCtrl ||
                controllers.ParamLoop ||
                controllers.paramLoop ||
                controllers.param_loop ||
                controllers.MouseTracking ||
                controllers.mouseTracking ||
                controllers.mouse_tracking ||
                controllers.ParamHit ||
                controllers.paramHit ||
                controllers.param_hit
            );

            const hasParamBreath = hasCoreParam('ParamBreath');

            // 后台总开关：默认开启
            const lpvBreathEnabled =
                typeof poilive2d_config === 'undefined' ||
                poilive2d_config.lpv_breath_enable !== '0';

            const useLPVBreath =
                lpvBreathEnabled &&
                hasParamBreath &&
                hasLPVSignal &&
                !autoBreathExplicitDisabled;

            let disableDepth = 0;

            // ==========================================
            // 非 LPV 模型：保留 Pixi 默认 Breath
            // ==========================================
            if (!useLPVBreath) {
                const control = {
                    mode: 'pixi-default',

                    setByCommand(enabled, source = '') {
                        if (!im.breath) return;
                        ...
                        im.breath.enabled = disableDepth === 0;

                        console.log(
                         //     `[Poi呼吸系统] ${modelName} Pixi Breath ${im.breath.enabled ? '启用' : '暂停'}，depth=${disableDepth}`,
                            source
                        );
                    },
                ...
                window._poiBreathControl = control;

                // console.log(
                //     `%c[Poi呼吸系统] ${modelName} -> Pixi 默认 Breath`,
            ...
            // ==========================================
            // LPV 模型：使用 ParamBreath Set 模式
            // ==========================================
            const cfg = {
                base: readPoiFloatConfig('lpv_breath_base', 0.5, 0.0, 1.0),
                amp: readPoiFloatConfig('lpv_breath_amp', 0.28, 0.0, 1.0),
                cycle: readPoiFloatConfig('lpv_breath_cycle', 5.2, 0.5, 30.0)
            };

            const range = getParamRange('ParamBreath');
            let breathT = 0;

            function syncPhaseFromCurrent() {
                const current = getCoreParam('ParamBreath', cfg.base);
                ...
                const n = (current - cfg.base) / cfg.amp;

                if (n >= -1 && n <= 1) {
                    breathT = Math.asin(n) / (Math.PI * 2) * cfg.cycle;
                } else {
                    breathT = -cfg.cycle / 4;
                }
            }

            syncPhaseFromCurrent();

            im.breath = {
                enabled: true,
                _poiMode: 'lpv-parambreath-set',

                updateParameters(coreModel, deltaTimeSeconds) {
                    if (!this.enabled) return;

                    let dt = deltaTimeSeconds;
                    ...
                    const rawValue = cfg.base + cfg.amp * Math.sin((breathT / cfg.cycle) * Math.PI * 2);
                    const finalValue = clamp(rawValue, range.min, range.max);

                    coreModel.setParameterValueById('ParamBreath', finalValue);
                }
            };

            const control = {
                mode: 'lpv-parambreath-set',
                ...
                setByCommand(enabled, source = '') {
                    ...
                    const shouldEnable = disableDepth === 0;

                    if (shouldEnable) {
                        syncPhaseFromCurrent();
                    }

                    im.breath.enabled = shouldEnable;

                    // console.log(
                    //     `[Poi呼吸系统] ${modelName} LPV Breath ${im.breath.enabled ? '启用' : '暂停'}，depth=${disableDepth}`,
                    //     source
                    // );
                },
            ...
            // console.log(
            //     `%c[Poi呼吸系统] ${modelName} -> LPV ParamBreath Set`,
        ...
        // ==========================================
        // ★ POI 命令系统：Command / PostCommand 分离
        // ==========================================
        function splitPoiCommands(commandText) {
        ...
        function setPoiEyeBlinkEnabled(enabled, source = '') {
            const eb = model.internalModel && model.internalModel.eyeBlink;

            if (!eb) {
                //console.log(`[Poi命令系统] eye_blink ${enabled ? 'enable' : 'disable'}：当前模型没有 eyeBlink`, source);
                return;
            }

            eb.enabled = !!enabled;

            // console.log(`[Poi命令系统] EyeBlink ${enabled ? '启用' : '暂停'}`, source);
        }

        function tryClearPoiExpression(source = '') {
            const mm = model.internalModel && model.internalModel.motionManager;
            const em = mm && mm.expressionManager;

            if (!em) {
                // console.log('[Poi命令系统] clear_exp：当前模型没有 expressionManager，跳过', source);
                return;
            }

            // pixi-live2d-display 没有稳定公开的 clearExpression API。
            // 这里只做安全尝试，不强行破坏内部状态。
            try {
                if (typeof em.resetExpression === 'function') {
                    em.resetExpression();
                    //  console.log('[Poi命令系统] clear_exp -> resetExpression()', source);
                    return;
                }

                if (typeof em.clearExpression === 'function') {
                    em.clearExpression();
                    //  console.log('[Poi命令系统] clear_exp -> clearExpression()', source);
                    return;
                }

                // console.log('[Poi命令系统] clear_exp 已识别，但当前库没有公开清除接口，暂时跳过', source);
            } catch (e) {
                //  console.warn('[Poi命令系统] clear_exp 执行失败，已跳过', e);
            }
        }

        function isPoiStateCommand(cmd) {
            const normalized = String(cmd || '').replace(/\s+/g, ' ').trim();

            return (
                /^auto_breath disable$/i.test(normalized) ||
                /^auto_breath enable$/i.test(normalized) ||
                /^eye_blink disable$/i.test(normalized) ||
                /^eye_blink enable$/i.test(normalized) ||
                /^clear_exp$/i.test(normalized)
            );
        }

        function runPoiStateCommand(cmd, phaseLabel = '') {
            const normalized = cmd.replace(/\s+/g, ' ').trim();

            if (/^auto_breath disable$/i.test(normalized)) {
                if (poiBreathControl && typeof poiBreathControl.setByCommand === 'function') {
                    poiBreathControl.setByCommand(false, phaseLabel);
                }
                return true;
            }

            if (/^auto_breath enable$/i.test(normalized)) {
                if (poiBreathControl && typeof poiBreathControl.setByCommand === 'function') {
                    poiBreathControl.setByCommand(true, phaseLabel);
                }
                return true;
            }

            if (/^eye_blink disable$/i.test(normalized)) {
                setPoiEyeBlinkEnabled(false, phaseLabel);
                return true;
            }

            if (/^eye_blink enable$/i.test(normalized)) {
                setPoiEyeBlinkEnabled(true, phaseLabel);
                return true;
            }

            if (/^clear_exp$/i.test(normalized)) {
                tryClearPoiExpression(phaseLabel);
                return true;
            }
        ...
        function runPoiCommandString(commandText, phase = 'command', options = {}) {
            const commands = splitPoiCommands(commandText);
            const skipStateCommands = options.skipStateCommands === true;

            commands.forEach(cmd => {
                const phaseLabel = `${phase}: ${cmd}`;

                // 状态命令：auto_breath / eye_blink / clear_exp
                // 如果前面 A0 已经执行过，这里就静默跳过，避免重复 disable。
                if (isPoiStateCommand(cmd)) {
                    if (!skipStateCommands) {
                        runPoiStateCommand(cmd, phaseLabel);
                    }
                    return;
                }

                // 路由命令：start_mtn / change_cos / open_url
                if (cmd.startsWith('start_mtn')) {
                   ...
                   //   console.log('[Poi命令系统] 未识别命令，已跳过:', cmd);
                }
            });
        }

        function getPoiPostCommandDelay(motionDef, hasMotionFile) {
            const explicitDelay =
                motionDef.PostDelay ??
                motionDef.post_delay ??
                motionDef.PostCommandDelay ??
                motionDef.post_command_delay;

            if (typeof explicitDelay === 'number' && Number.isFinite(explicitDelay)) {
                return Math.max(0, explicitDelay);
            }

            const duration =
                motionDef.Duration ??
                motionDef.duration ??
                motionDef.TextDuration ??
                motionDef.text_duration;

            if (typeof duration === 'number' && Number.isFinite(duration)) {
                return Math.max(0, duration);
            }

            // 没有动作文件，通常只是表情/眨眼命令，给短延迟
            if (!hasMotionFile) return 1200;

            // 有动作文件但 JSON 没写时，给保守默认值
            return 2500;
        }

        function schedulePoiPostCommand(commandText, motionDef, hasMotionFile) {
            if (!commandText) return;

            const delay = getPoiPostCommandDelay(motionDef, hasMotionFile);

            console.log(`[Poi命令系统] PostCommand 将在 ${delay}ms 后执行:`, commandText);

            setTimeout(() => {
                runPoiCommandString(commandText, 'post_command');
            }, delay);
        }
        // ==========================================
        ...
        function executeVTSAction(groupName, specificName = null, isTopLevel = false) {
            ...
            const mCommand = motionDef.Command || motionDef.command || '';
            const mPostCommand = motionDef.PostCommand || motionDef.post_command || '';
            ...
            const hasMotionFile = !!mFile;

            // --- A0. 动作开始前先执行状态命令 ---
            // auto_breath disable 必须尽量早执行，否则 breath 会在 motion 后继续覆盖 ParamBreath。
            if (mCommand) {
                splitPoiCommands(mCommand).forEach(cmd => {
                    if (isPoiStateCommand(cmd)) {
                        runPoiStateCommand(cmd, `command_before_motion: ${cmd}`);
                    }
                });
            }
            ...
            if (engineGroup && mFile) {
                try {
                    const ret = model.motion(groupName, finalIndex);

                    if (ret && typeof ret.then === 'function') {
                        motionPromise = ret.catch(e => {
                         //     console.warn('[Poi动作系统] motion promise 异常，已吞掉:', e);
                        });
                    }
                } catch (e) {
                  //    console.warn('[Poi动作系统] model.motion 执行失败:', e);
                }

            } else if (!mFile && !mCommand && !mPostCommand && !mSound && !mText && (!mChoices || mChoices.length === 0)) {
            ...
            if (mCommand) {
                // 状态命令已经在 A0 执行过，这里只处理 start_mtn / change_cos / open_url
                runPoiCommandString(mCommand, 'command', {
                    skipStateCommands: true
                });
            }

            if (mPostCommand) {
                schedulePoiPostCommand(mPostCommand, motionDef, !!mFile);
            }
        ...
        const handlePointerUp = (e) => {
            ...
            if (pointerDownContext) {
                if (!pointerDownContext.isDragging) {
...
function showConsoleTips(content) {
    ...
    console.log("%cLive2d%cPoi模型" + content + "完成", style_green, style_green_light);
