...
function live2d_scripts()
{
	...
	wp_enqueue_style(
		'live2d-base',
		LIVE2D_URL . '/live2d/css/live2d.css',
		array(),
		LIVE2D_VERSION,
		'all'
	);
	...
	wp_enqueue_script(
		'live2d-jquery',
		LIVE2D_URL . '/live2d/js/jquery.min.js',
		array('jquery'),
		LIVE2D_VERSION,
		true
	);
	...
	wp_enqueue_script(
		'pixi-js',
		LIVE2D_URL . '/live2d/js/pixi.min.js',
		array(),
		'7.4.2',
		true
	);
	...
	wp_enqueue_script(
		'live2d-core-v2',
		LIVE2D_URL . '/live2d/js/live2d-v2core.min.js',
		array(),
		null,
		true
	);
	...
	wp_enqueue_script(
		'live2d-core-v4',
		LIVE2D_URL . '/live2d/js/live2dcubismcore.min.js',
		array(),
		null,
		true
	);
	...
	wp_enqueue_script(
		'pixi-live2d-display',
		LIVE2D_URL . '/live2d/js/pixi-live2d-display.min.js',
		array('pixi-js', 'live2d-core-v2', 'live2d-core-v4'),
		null,
		true
	);
	...
	wp_enqueue_script(
		'poilive2d-message',
		LIVE2D_URL . '/live2d/js/live2d-message' . $script_suffix . '.js',
		array('jquery'),
		LIVE2D_VERSION,
		true
	);
	...
	wp_enqueue_script(
		'poilive2d-ctrl',
		LIVE2D_URL . '/live2d/js/live2d-ctrl' . $script_suffix . '.js',
		array('pixi-live2d-display', 'poilive2d-message'),
		LIVE2D_VERSION,
		true
	);
	...
	wp_enqueue_script(
		'live2d-run',
		LIVE2D_URL . '/live2d/js/run_local' . $script_suffix . '.js',
		array('live2d-jquery', 'poilive2d-ctrl'),
		LIVE2D_VERSION,
		true
	);
}
...
function live2d_head()
{
    if (!wp_is_mobile()) {
        ...
        echo 'var poilive2d_api_url = "' . esc_url_raw(rest_url('poilive2d/v1/model')) . '";';        
        ...
        foreach ($model_dirs as $dir) {
            ...
            if (count($v2_models) > 0) {
                ...
                if ($valid_tex_count <= 1) {
                    ...
                    // 如果有多件衣服，必须走老路子，传衣服数量给前端，让 live2d-v2api.php 负责动态换装
        ...
        $css .= "#landlord { width: {$r_w}px !important; height: {$r_h}px !important; {$h_css} {$v_css}  transform: translateZ(0); }";
        ...
        // --- 气泡样式设定 ---
        $b_w = isset($settings['bubble_size']['w']) && intval($settings['bubble_size']['w']) > 0 ? intval($settings['bubble_size']['w']).'px' : 'calc(100% + 20px)';
        ...
        $b_y_offset = isset($settings['bubble_y_offset']) ? intval($settings['bubble_y_offset']) : 0;
        $b_bottom = ($r_h - 50) + $b_y_offset;
        ...
        // --- 按钮样式设定 ---
        $btn_size = isset($settings['btn_size']) && is_array($settings['btn_size'])? $settings['btn_size']: array('w' => 60, 'h' => 20);

        $btn_w = isset($btn_size['w']) ? intval($btn_size['w']) : 60;
        $btn_h = isset($btn_size['h']) ? intval($btn_size['h']) : 20;

        $btn_font_size = isset($settings['btn_font_size']) ? intval($settings['btn_font_size']) : 12;
        ...
        $css .= ".l2d-action, .l2d-action-L, .show-button {
                ...
                line-height: {$btn_h}px !important;
