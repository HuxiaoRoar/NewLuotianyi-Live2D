<?php
    ...
    function live2d_head()
    {
        if (!wp_is_mobile()) {
            ...
            echo '<script type="text/javascript">var live2d_Path = "' . LIVE2D_URL . '/live2d/model/天依/";var message_Path = "' . LIVE2D_URL . '/live2d/";var home_Path = "' . home_url() . '/";'.$nohitokoto.$nospecialtip.$nocatalog.$localkoto.'</script>';
            ...
            if (!get_option('live2d_bgcolor')) {
                ...
                $bgcolor = hex2rgb(get_option('live2d_bgcolor'));
            }
            if (!get_option('live2d_fontcolor')) {
                $fontcolor = "255,255,255";
            } else {
                $fontcolor = hex2rgb(get_option('live2d_fontcolor'));
            }
            echo "<style>.message{border-color:rgba($maincolor,.5);box-shadow:0 3px 15px 2px rgba($maincolor,.6);background-color:rgba($bgcolor,.9);color:rgba($fontcolor,1);}.l2d-action-L, .l2d-action, .show-button{border-color:rgba($maincolor,.5);box-shadow:0 3px 15px 2px rgba($maincolor,.6);background:rgba($bgcolor,.9);color:rgba($fontcolor,1);}.l2d-action-L:hover, .l2d-action:hover, .show-button:hover{border-color:rgba($maincolor,1);background:rgba($bgcolor,0.5);color:rgba($fontcolor,0.8);}</style>";
        }
    }
